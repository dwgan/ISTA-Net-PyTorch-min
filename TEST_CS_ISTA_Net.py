...
parser.add_argument('--noise_mode', type=str, default='cs', help='perturbed cs mode')
...
class BasicBlock(torch.nn.Module):
    ...
    def forward(self, x, Phi, Phix, PhiTPhi, PhiTb):
        # for perturbed constrained term
        if args.noise_mode == "pcs":
            x_trace = torch.trace(x.t() @ x)
            Phix_trace = torch.trace((x @ Phi.t() - Phix).t() @ (x @ Phi.t() - Phix))
            x_grad = ((1 + x_trace) * (x @ PhiTPhi - PhiTb) + Phix_trace * x ) / (1 + x_trace)
        # for unperturbed constrained term
        else:
            x_grad = torch.mm(x, PhiTPhi) - PhiTb

        x = x - self.lambda_step * (x_grad)
...
class ISTANet(torch.nn.Module):
    ...
    def forward(self, Phix, Phi, Qinit):
        ...
        for i in range(self.LayerNo):
            [x, layer_sym] = self.fcs[i](x, Phi, Phix, PhiTPhi, PhiTb)
...
with torch.no_grad():
    for img_no in range(ImgNum):
        ...
        # generate noise
        E = noise_generate_E(noise_snr, Phi, batch_x)
        e = noise_generate_e(noise_snr, Phi, batch_x)

        Phix = torch.mm(batch_x, torch.transpose(Phi + E, 0, 1))
        ...
        # 填充尺寸 Ipad: 264x264
        # 向量形式 Icol: 64x1089
        # 原始数据 batch_x: 64x1089
        # 采样矩阵 Phi: 272x1089
        # 测量向量 Phix: 64x272
        # 压缩比例 ratio: size(Phix)/size(batch_x)=25%
        # Qinit: 1089x272
        # x_output: 64x1089
        # Prediction_value: 264x264
        # X_rec: 256x256
        ...
        # plt.imshow(X_rec*255, cmap='gray')
        # plt.title('Reconstruction')
        # # Plotting the second image
        # plt.subplot(1, 2, 2)  # 1 row, 2 columns, second subplot
        # plt.imshow(Iorg, cmap='gray')
        ...
        # plt.clf()
        # axs[0].imshow(X_rec*255, cmap='gray')  # Draw new image
        # axs[0].set_title('Reconstruction')
        #
        # axs[1].imshow(Iorg, cmap='gray')  # Draw new image
...
# print('\n')
output_data = "CS ratio is %d, Avg PSNR/SSIM for %s is %.2f/%.4f, Epoch number of model is %d" % (cs_ratio, args.test_name, np.mean(PSNR_All), np.mean(SSIM_All), epoch_num)
print(output_data)
