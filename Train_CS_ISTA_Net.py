...
parser.add_argument('--noise_mode', type=str, default='pcs', help='perturbed cs mode')
...
log_file_name = "./%s/Log_CS_ISTA_Net_layer_%d_group_%d_ratio_%d_lr_%.4f_snr_%ddB_lw_%.4f_%s.txt" % (args.log_dir, layer_num, group_num, cs_ratio, learning_rate, args.noise_snr, args.loss_weight, args.noise_mode)
...
for epoch_i in range(start_epoch+1, end_epoch+1):
    for data in rand_loader:
        ...
        gamma = torch.Tensor([args.loss_weight]).to(device)
